...
Instagram [@dear.couple.kr](https://instagram.com/dear.couple.kr) 과 Threads @dear.couple.kr 에
**매일 게시물 하나를 자동으로 올리는** 저장소. 콘텐츠(이미지·캡션)를 여기에 두고, 저장소가 이미지 호스팅도 겸한다.

원본 문서는 앱 저장소 옆 `Dear/Marketing/` 에 있다 — 무엇을 올릴지는 `Dear-Features.md`,
어떻게 말할지는 `Dear-Brand-Guide.md`, 계정·API 기록은 `Dear-SNS-Setup.md`.
...
> ⚠️ GitHub Actions 의 cron 은 정시에 돌지 않는다. 부하가 몰리면 수십 분에서 수 시간까지 밀린다.
> 21:00 으로 잡아 둔 건 그 여유를 감안한 것이다. 정확한 시각이 필요하면 수동 실행을 쓴다.
...
```json
{
  ...
  "format": "A",
  "ko": {
    "images": ["d01_1.png", "d01_2.png"],
    "caption": "하루 한 질문, 둘만의 대화가 쌓여요.\n\n…\n\n#커플 #커플스타그램 #Dear",
    "threads": "질문을 하루에 하나만 보낼 수 있게 만들었어요. …"
  }
}
```

- `ko.images` — `images/` 안의 파일 이름. 1장이면 단일 게시물, 2~10장이면 캐러셀
- `ko.caption` — Instagram 캡션. 2,200자 · 해시태그 30개 이내
- `ko.threads` — Threads 문장(500자 이내). 없으면 Threads 는 그날을 건너뛴다
...
for f in queue/*.json; do
  jq -e '.date and .day and .ko.caption and (.ko.images|length>0)' "$f" >/dev/null || echo "FAIL $f"
done
jq -r '.ko.images[]' queue/*.json | sort -u | while read -r i; do
...
`https://raw.githubusercontent.com/<owner>/Dear-sns/main/images/d01_1.png` 가 인증 없이 열린다.
비공개면 이미지 확인 단계에서 404 로 실패한다.
...
```bash
# 특정 날짜를 지금 올리기 — Actions 탭에서 date 입력 + dry_run=false
# 로컬에서 점검만 (발행 안 함)
IG_USER_ID=… IG_TOKEN=… \
...
발급일은 `state/tokens.json` 에 기록되고 갱신될 때마다 워크플로가 커밋한다.
...
| 예약 시각에 안 돎 | cron 지연이 흔하다. 몇 시간 기다려 보고, 그래도 없으면 수동 실행 |
