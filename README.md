...
| `log.md` push 거절·충돌로 워크플로 실패 | **게시물은 이미 올라간 상태다.** 두 워크플로가 겹쳐 돌면 같은 줄 끝에서 부딪힌다 — `.gitattributes` 의 `merge=union` + 3회 재시도로 자동 해결되지만, 그래도 실패하면 `git pull` 후 기록만 손으로 한 줄 넣는다. 워크플로를 다시 돌리면 **중복 게시**된다 |
